...
where abs(p1.unitprice - p2.unitprice) <=10 group by p1.productname order by similar_price_count desc;

select s.country, count(distinct p.categoryid) from suppliers s join products p on s.supplierid = p.supplierid
group by s.country order by count(p.categoryid) desc limit 1;
...
select current_date - hiredate as hire_days, firstname, extract(year from age(current_date, hiredate)) as age from employees
...
join employees e2 on e2.birthdate > e1.birthdate
